...
const BUCKET = 'takenoko-test-images';
...
exports.handler = (event, context, callback) => {
  ...
  if (response.status == 404) {
    ...
    S3.getObject({ Bucket: BUCKET, Key: originalKey }).promise()
      //.then(data => wasm.resize(data.Body.toString("base64"), width, height))
      .then(data => wasm.resize(data.Body, width, height))
      ...
      .then(result => {
        ...
        //let buffer = wasm.resize(data.Body, width, height);
        ...
        response.status = 200;
        response.body = result//buffer.toString('base64');
